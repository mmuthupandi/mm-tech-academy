...
export const EnrollmentModal = () => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('open-enrollment-modal', handleOpen);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await addExampleDocument('enrollments', {
        ...formData,
      ...
      setIsOpen(false);
      setLoading(false);
      setTimeout(() => window.dispatchEvent(new CustomEvent('show-toast', { detail: { message: 'Enrollment request submitted successfully! We will contact you shortly to finalize your registration.', type: 'success' } })), 10);
    } catch (error) {
      console.error(error);
      setLoading(false);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: '0.9rem', color: '#64748b', marginBottom: '1.5rem', lineHeight: 1.5 }}>
          Secure your spot in our upcoming cohort. Fill out the details below.
        </p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <select required value={formData.course} onChange={e => setFormData({ ...formData, course: e.target.value })}
              ...
              <option value="">Course to Enroll In</option>
          ...
          <button type="submit" disabled={loading} style={{
            ...
            {loading ? 'Submitting...' : 'Confirm Enrollment Request'}
          </button>
        </form>
      </div>
    </div>
